...
- 미작성 활동일지가 있으면 새 예약을 할 수 없고, 활동일지를 제출한 뒤 예약할 수 있습니다.
- 엑셀로 추가한 오늘 이후 정기대관은 `User ID`, 날짜, 종료 시간이 있으면 예약 ID와 `Pending` 상태를 자동 보완합니다.
- `데일리` 계정은 활동일지를 수기로 작성하므로 팝업과 미작성 예약 제한에서 제외합니다.
